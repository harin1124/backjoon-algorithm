...
/**
 * @title 5. 쇠막대기
 * @desc 여러 개의 쇠막대기를 레이저로 절단하려고 한다. 효율적인 작업을 위해서 쇠막대기를 아래에서 위로 겹쳐 놓고,<br>
 ...
 *        아래 그림은 위 조건을 만족하는 예를 보여준다. 수평으로 그려진 굵은 실선은 쇠막대기이고, 점은 레이저의 위치,<br>
 *        수직으로 그려진 점선 화살표는 레이저의 발사 방향이다.<br>
 ...
 * @studyStartDate 2023-03-04
 * @studyEndDate 2023-03-04
 */
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=0; i<s.length(); i++){
			if(s.charAt(i) == 40){
				...
				stack.pop();
				if(s.charAt(i-1) == 40){
					// 닫는 괄호의, 바로 전 문자열이 (이면 레이저이다.
					// 즉, 레이저를 만나면 막대기를 잘라버리므로, 스택에 담긴 막대기의 수(스택의 사이즈) 만큼 자른다.
					result += stack.size();
				} else {
					//
					result++;
