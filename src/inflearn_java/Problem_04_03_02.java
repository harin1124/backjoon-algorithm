 ...
 */
public class Problem_04_03_02 {
	public static void main(String[] args) throws Exception {
		// n 제곱이 아닌 n만큼만 돌도록 처리 + getOrDefault
		...
		for(i=0; i<arr.length; i++){
			...
			// 인덱스 추가 또는 증가 처리
			if(map.containsKey(arr[range+i])){
				map.put(arr[range+i], map.get(arr[range+i])+1);
			} else {
				map.put(arr[range+i], 1);
			}
			bw.write(map.size()+" ");
		}
		bw.close();
