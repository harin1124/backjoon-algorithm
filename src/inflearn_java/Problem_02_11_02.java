 ...
 */
public class Problem_02_11_02 {
	public static void main(String[] args) throws Exception {
		// 문제를 풀지 못해 답을 보고 푼 풀이
		...
		for(x=1; x<=n; x++){
			cnt = 0;
			for(y=1; y<=n; y++){
				System.out.printf("panel[%d][%d] = %d", x, y, arr[x][y]);
				System.out.println();
				for(k=1; k<=5; k++) {
					System.out.printf("\tpanel[%d][%d] = %d\t\tpanel[%d][%d] = %d", x, k, arr[x][k], y, k, arr[y][k]);
					System.out.println();
					if(arr[x][k] == arr[y][k]){
						cnt++;
						break;
